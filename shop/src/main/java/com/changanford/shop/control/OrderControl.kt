    ...
    fun bindingGoodsInfo(dataBinding: InItemOrderGoodsBinding, itemBean: OrderItemBean) {
        dataBinding.apply {
            ...
            if (itemBean.skuOrderVOList == null || itemBean.skuOrderVOList?.size == 1) {
                val skuItem =
                    if (itemBean.skuOrderVOList != null && itemBean.skuOrderVOList!!.size == 1) itemBean.skuOrderVOList?.get(
                        0
                    ) ?: OrderSkuItem()
                    else OrderSkuItem(
                        skuImg = itemBean.skuImg,
                        spuName = itemBean.spuName,
                        busSourse = itemBean.busSourse
                    )
                ...
                recyclerView.apply {
                    if (!TextUtils.isEmpty(skuItem.specifications)) {
                        ...
                        adapter = OrderGoodsAttributeAdapter().apply {
                            val specifications =
                                skuItem.specifications?.split(",")?.filter { "" != it }
        ...
        item?.apply {
            if ("2" == busSourse) {
                ...
                JumpUtils.instans?.jump(
                    1,
                    String.format(
                        MConstant.H5_SHOP_BARGAINING,
                        mallMallHaggleSpuId,
                        mallMallHaggleActivityId
                    )
                )
            } else {
                // TODO 再次购买的逻辑。
