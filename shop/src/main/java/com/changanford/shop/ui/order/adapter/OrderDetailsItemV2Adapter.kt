            ...
            if (item.mallRefundStatus != null && !TextUtils.isEmpty(item.mallRefundStatus)) {
                ...
                when (item.mallRefundStatus) {
                    "ON_GOING" -> {
                        tvSaleHandler.visibility = View.VISIBLE
                        tvSaleHandler.text = "退款中"
                        orderStatusListener.orderStatusShow(
                            item,
                            needShow = false,
                            isShowBack = false
                        )
                ...
                if (!TextUtils.isEmpty(busSource)) {
                    if (busSource == "HAGGLE") {
                        JumpUtils.instans?.jump(
                            1,
                            String.format(MConstant.H5_SHOP_BARGAINING, item.mallMallspuId)
                        )
                    } else {
...
//                        }
                        ...
                        bundle.putBoolean("isShowBack", itemStatus == "退款关闭")
                        item.mallOrderSkuId?.let {
...
//                )
    ...
    interface OrderStatusListener {
        ...
        fun orderStatusShow(item: OrderItemBean, needShow: Boolean, isShowBack: Boolean)
    }
}
