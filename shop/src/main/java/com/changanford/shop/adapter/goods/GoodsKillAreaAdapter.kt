...
class GoodsKillAreaAdapter(val viewModel: GoodsViewModel) :
    BaseQuickAdapter<GoodsItemBean, BaseDataBindingHolder<ItemGoodsKillAreaBinding>>(R.layout.item_goods_kill_area),
    LoadMoreModule {
    @SuppressLint("SetTextI18n")
    override fun convert(
        holder: BaseDataBindingHolder<ItemGoodsKillAreaBinding>,
        item: GoodsItemBean
    ) {
        holder.dataBinding?.apply {
            ...
            item.stockProportion = WCommonUtil.getPercentage(
                ...
                item.stockPlusSalesCount.toDouble(),
                0
            )
                    ...
                    override fun onFinish(code: Int) {
                        if (0 == code) {
                            ...
                            ToastUtils.showLongToast(
                                context.getString(R.string.prompt_set_setNotic),
                                context
                            )
            ...
            4 -> viewModel.setKillNotices("CANCEL", item.mallMallSpuSeckillRangeId, object :
                ...
                override fun onFinish(code: Int) {
                    if (0 == code) {
                        ...
                        dataBinding.btnStates.setStates(getKillStates(item))
                        ToastUtils.showLongToast(
                            context.getString(R.string.prompt_cancel_setNotic),
                            context
                        )
                        notifyDataSetChanged()
                    }
