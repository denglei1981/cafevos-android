...
class GoodsKillAreaTimeAdapter(var selectPos: Int, val listener: SelectTimeBackListener) :
    BaseQuickAdapter<SeckillTimeRange1, BaseDataBindingHolder<ItemKillTimeBinding>>(R.layout.item_kill_time),
    LoadMoreModule {
    ...
    override fun convert(
        holder: BaseDataBindingHolder<ItemKillTimeBinding>,
        item: SeckillTimeRange1
    ) {
        ...
        if (dataBinding != null) {
            ...
            if (selectPos == position) {
                dataBinding.layoutRoot.setBackgroundResource(R.drawable.bg_081700f4_12)
                dataBinding.tvTime.setTextColor(
                    ContextCompat.getColor(
                        context,
                        R.color.color_1700f4
                    )
                )
                dataBinding.tvState.setTextColor(
                    ContextCompat.getColor(
                        context,
                        R.color.color_1700f4
                    )
                ...
                dataBinding.layoutRoot.setBackgroundResource(R.drawable.bg_084a_12)
                dataBinding.tvTime.setTextColor(ContextCompat.getColor(context, R.color.color_d916))
                dataBinding.tvState.setTextColor(ContextCompat.getColor(context, R.color.color_9916))
            }
    ...
    private fun switchItem(
        itemBinding: ItemKillTimeBinding,
        position: Int,
        item: SeckillTimeRange1
    ) {
        selectPos = position
