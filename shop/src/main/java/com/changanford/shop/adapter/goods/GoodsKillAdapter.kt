...
import com.changanford.common.util.ext.setTopCircular
...
class GoodsKillAdapter :
    BaseQuickAdapter<GoodsItemBean, BaseDataBindingHolder<ItemKillGoodsBinding>>(R.layout.item_kill_goods),
    LoadMoreModule {
...
//            )
            ...
            val robbedPercentage =
                WCommonUtil.getPercentage(sekillCount.toDouble(), totalStock.toDouble(), 0)
            item.robbedPercentage = robbedPercentage
            imgCover.load(item.getImgPath())
            tvChangeNum.text = "已兑换${item.sekillCount}件"
...
//            }
//            tvSeckillStatus.setBackgroundResource(
//                if ("ON_GOING" == status) R.drawable.shadow_cc00095b_2dp
//                else R.drawable.shadow_6600095b_2dp
//            )
            ivType.setImageResource(getsTopBg(item.seckillStatus))
    ...
    private fun getsTopBg(status: String): Int {
        return when (status) {
            "NOT_BEGIN" -> R.mipmap.ic_shop_type_no_begin
            "ON_GOING" -> R.mipmap.ic_shop_type_ing
            else -> R.mipmap.ic_shop_type_over
        }
    }
}