...
public class RangeSlider extends ViewGroup {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                int left = mLeftThumb.getRangeIndex();
                int right = mRightThumb.getRangeIndex();
                Log.e("left+right", left + "-----" + right);
                mLastX = mOriginalX = x;
                mIsDragging = false;
    ...
    private void moveLeftThumbByPixel(int pixel) {
        ...
        if (x > start && x < end && x < mRightThumb.getX() - mThumbWidth) {
            if ((mRightThumb.getX() - x) < (getRangeLength() / mTickCount * 3)) {
                return;
            }
            mLeftThumb.setX(x);
    ...
    private void moveRightThumbByPixel(int pixel) {
        ...
        float left = mLeftThumb.getX();
        float interval = getIntervalLength();
        float start = mTickStart / mTickInterval * interval;
        float end = mTickEnd / mTickInterval * interval;

        if (x > start && x < end && x > (mLeftThumb.getX() + mThumbWidth)) {
            Log.e("right", x + "---" + getRangeLength() + "----" + mTickCount * 3 + "----" + (getRangeLength() / mTickCount) * 3);
            float maxlenth = getRangeLength() / mTickCount * 3;
            if (maxlenth > (x - left)) {
                return;
            }
