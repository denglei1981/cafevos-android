...
//        MainApplication.setConsoleText("onNotificationReceivedInApp ： " + " : " + title + " : " + summary);
    ...
    public void onNotificationOpened(Context context, String title, String summary, String extraMap) {
        ...
        if (extraMap != null && !extraMap.isEmpty()) {
            try {
                JSONObject jsonObject = JSON.parseObject(extraMap);
                String jumpDataType = jsonObject.getString("jumpDataType");
                String jumpDataValue = jsonObject.getString("jumpDataValue");
                JumpUtils.getInstans().jump(Integer.valueOf(jumpDataType), jumpDataValue);
            } catch (Exception e) {
                e.printStackTrace();
                Log.e(REC_TAG, "数据解析错误");
            }
        }
