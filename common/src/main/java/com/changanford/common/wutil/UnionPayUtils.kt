...
object UnionPayUtils {
    ...
    fun goUnionPay(
        ...
        serverMode: String? = "00"
    ) {
        if (appPayRequest == null) return
        when (type) {
            //支付宝小程序支付
            1 -> {
                if (AppUtils.checkAliPayInstalled(activity)) payAliPayMiniPro(
                    activity,
                    appPayRequest
                )
...
//        }
    ...
    private fun openWX(mContext: Context) {
        val packageManager: PackageManager = BaseApplication.curActivity.packageManager
