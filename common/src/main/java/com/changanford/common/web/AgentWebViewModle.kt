...
//            })
    ...
    fun initLocationOption() {
        try {
            ...
            val locationClient = LocationClient(BaseApplication.INSTANT)
...
//        }
    ...
    //获取用户认证车辆列表
    fun getMyBindCarList(myBindCarList: MyBindCarList) {
        viewModelScope.launch {
            val request = fetchRequest {
                val body = HashMap<String, Any>()
                val rkey = getRandomKey()
                apiService.myBindCarList(body.header(rkey), body.body(rkey))
            }
            if (request.code == 0) {
                request.data?.let {
                    val gson = Gson()
                    val string = gson.toJson(it)
                    myBindCarList.myBindCarList(string)
                }
            } else {
                request.msg.toast()
            }
...
interface MyBindCarList {
    fun myBindCarList(string: String)
}